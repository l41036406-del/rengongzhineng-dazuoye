...
--------------------------------------------------------------------
    ...
    4. 单场预测    : 选择球队与阵型,输出模型共识、胜平负概率与阵型影响
...
--------------------------------------------------------------------
    ...
    1. 默认单场预测使用多个分类模型的加权共识,并加入小比例 ELO
       先验校准,避免某一个模型的边缘判断直接成为最终结论。

    2. 页面支持 4-3-3、4-2-3-1、4-4-2、3-5-2、5-3-2、3-4-3
       和 4-1-4-1 七种阵型。

    3. 当前数据集没有逐场阵型、首发球员和伤停字段,因此阵型影响采用
       独立、小幅且可解释的战术情景修正,不属于历史模型的训练特征。

    4. 页面会同时展示基础概率、阵型修正、模型投票和最终概率,
       便于追溯结果。

    5. 中国 vs 阿根廷示例中,默认共识结果为阿根廷胜。随机森林单模型
       仍可能给出平局,但该判断仅获得 7 个模型中的 1 票,且与阿根廷胜
       的概率差距很小,因此不适合作为默认结论。
